	...
	<body>	
        ...
        if ( $start <= $now && $now <= $end ){
            ...
            if (mysql_num_rows($result) > 0) {
                ...
                echo '<tr><th>Laboratório '.$row['laboratorio'].' - '.$row['bloco'].'</th></tr>'; 
                }
            } 
            else {
                echo '<td>Nenhum disponivel</td>';
        }  
        }
        ...
        if ( $start <= $now && $now <= $end ){
            ...
            if (mysql_num_rows($result) > 0) {
                while($row = mysql_fetch_assoc($result)) {
                echo '<div class="dispo">Laboratório '.$row['laboratorio'].' - '.$row['bloco'].'</div>';
                }
            }   
        }
